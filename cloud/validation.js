...
module.exports = {
  ...
  validateStyle: function(object) {
    ...
    result = tv4.validateMultiple(model, constraints.definitions.StyleConfig);
    //check validate
    if(!result.valid){
      // console.log(result);
      var endResult = customeResult(result.errors);
  ...
  validateItem: function(object) {
    ...
    if(!result.valid){
      var endResult = customeResult(result.errors);
      // var endResult1 = groupingItemConfigErrors(endResult);
...
function customeResult(jsonError) {
